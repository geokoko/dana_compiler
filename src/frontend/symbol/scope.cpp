#include "scope.hpp"

#include <utility>

Scope::Scope(Scope* parent)
	: parent_(parent) {}

// Declare symbol in current scope (aka insert into symbol table)
InsertResult Scope::declare(Symbol* symbol) {
	if (!symbol) {
		return {};
	}

	const std::string& id = symbol->getName();
	auto [it, inserted] = table_.emplace(id, symbol);
	return InsertResult{it->second, inserted};
}

// Lookup symbol in current scope only
LookupResult Scope::lookupLocal(const std::string& id) const {
	auto it = table_.find(id);
	if (it == table_.end()) {
		return {};
	}
	return LookupResult{it->second, this};
}

// Lookup symbol in current scope and parent scopes
LookupResult Scope::lookup(const std::string& id) const {
	for (const Scope* scope = this; scope != nullptr; scope = scope->parent_) {
		// Look in current scope
		auto result = scope->lookupLocal(id);
		if (result.symbol) {
			return result;
		}
	}
	return {};
}

// Get parent scope
Scope* Scope::parent() const noexcept {
	return parent_;
}

// Get all symbols in the current scope
const std::unordered_map<std::string, Symbol*>& Scope::symbols() const {
	return table_;
}
