...
bool SemanticPass::signaturesMatch(const HeaderInfo& info, const Symbol* symbol) {
...
bool SemanticPass::validateDimension(const std::optional<int>& dim, bool allowUnsized,
...
SemaTypePtr SemanticPass::resolveType(const Type& node, SemContext& context, bool allowUnsizedFirst) {
...
void SemanticPass::visit(Type& n) { resolveType(n, context_); }
...
void SemanticPass::visit(Header&) {
	// Header processing is now done inline via extractHeaderInfo
}

SemanticPass::HeaderInfo SemanticPass::extractHeaderInfo(Header& header) {
	std::unordered_set<std::string> seen;
	HeaderInfo info;
	info.name = header.identifier();
	info.loc = header.loc;
	const auto returnType = header.returnType();
...
void SemanticPass::visit(FuncDecl& n) {
	...
	auto info = extractHeaderInfo(*header);
...
void SemanticPass::visit(FuncDef& n) {
	...
	if (symbol) {
		...
		// Use existing forward-declared symbol
	...
	symbol->setDefiningFunc(currentFunction()
		? static_cast<FuncSymbol*>(currentFunction()->symbol)
		: nullptr);
	...
	// If forward-declared, clear old orphaned params and replace with new ones
	...
	// Type-check the function body while scope is still open
...
void SemanticPass::enterFunction(FunctionFrame frame) {
	functionStack_.push_back(std::move(frame));
}

void SemanticPass::leaveFunction() {
	if (!functionStack_.empty()) {
		functionStack_.pop_back();
	}
}

SemanticPass::FunctionFrame* SemanticPass::currentFunction() {
